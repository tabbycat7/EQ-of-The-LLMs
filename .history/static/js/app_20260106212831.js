...
async function startPhilosophy() {
    try {
        ...
        if (!response.ok) throw new Error('启动竞技场失败');
        ...
        document.getElementById('philosophy-input-section').style.display = 'block';
...
async function sendPhilosophyMessage() {
    ...
    sendBtn.disabled = true;
    // 发送后隐藏输入区域，直到本轮投票完成
    const philosophyInputSection = document.getElementById('philosophy-input-section');
...
function setupPhilosophyVoteButtons(roundEl) {

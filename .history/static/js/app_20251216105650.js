...
// 初始化应用
document.addEventListener('DOMCo    ntentLoaded', async () => {
    // 加载可用模    型
    await loadModels();
...
async function loadModels() {
    try {
        ...
        const dat        a = await response.json();
...
function setupModeSelector() {
    ...
    modeButtons.forEach(btn => {
        btn.addEventLis            tener('click', () => {
            const mode = btn.dataset.mode;

            // 更新按钮状态
            modeButtons.forEach(b => b.clas            sList.remove('active'));
...
function setupBattleMode() {
    ...
    c    onst voteButtons = document.querySelectorAll('.battle-vote-btn');
    // 记录对战模式中的输入区域容器
    ...
    input.a    ddEventListener('keypress', (e) => {
...
asyn        c function startBattle() {
    try {
        ...
        battleSessionId = data.session        _id;
        ...
        document.getElementById('re        veal-section').style.display = 'none';
...
async function sendBattleMessage() {
    ...
    const send        Btn = document.getElementById('battle-send-btn');
    ...
    try {
        ...
        document.ge        tElementById('response-b').innerHTML = '<div         class="loading">思考中...</div>';
        ...
        if (battleInp        utSection) {
            battleInputSection.style.display = 'block';
        }
        sendBt        n.disabled = false;
    }
}
...
async function submitVote(winner) {
    try {
        ...
        document.getElementById('revea    l-section').style.display = 'block';
...
function setupSideBySideMode() {
    ...
    // 监听模型选择变化
    const modelASelect = docume    nt.getElementById('sidebyside-model-a');
...
function updateSideBySideHeaders() {
    const model    AId = document.getElementById('sidebyside-model-a').value;
    const modelBId = document.getElemen    tById('sidebyside-model-b').value;

    const modelA = availableModels.find(m => m.id === modelAId);
    const modelB = availableModels.find(m => m.id === modelBId);

    if (modelA) {
        document.getElementById('sideb        yside-header-a').textContent = modelA.name;
    }
    if (modelB) {
...
async function sendSideBySideMessage() {
    ...
    const modelAId = document.getElementBy        Id('sidebyside-model-a').value;
    const modelBId = d        ocument.getElementById('sidebyside-model-b').value;

    const sendBtn = document.getElement        ById('sidebyside-send-btn');
    sendBtn.disabled = true;
    // 发送后立即隐藏用户输入区域，等待本轮投票完成后再显示
    ...
    try {
        ...
        // 发送失败时重新显示输入区域，允许用户重试
        if (sideBySideInputSection) {
            sideBySideInputSection.style.display = 'block';
        }
    } finally {
...
async function loadLeaderboard() {
    ...
    try {
        ...
        container.i    nnerHTML = '<div class="empty-state">加载失败，请重试</div>';
    }
}

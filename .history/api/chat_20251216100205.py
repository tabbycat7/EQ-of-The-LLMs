...
async def side_by_side_chat(
    ...
    try:
        print(f"[Side-by-Side] 收到请求: model_a={request.model_a_id}, model_b={request.model_b_id}, message={request.message[:50]}...")
        
        # 验证模型是否存在
        print(f"[Side-by-Side] 验证模型...")
        model_a_info = model_service.get_model_info(request.model_a_id)
        model_b_info = model_service.get_model_info(request.model_b_id)
        
        if not model_a_info:
            print(f"[Side-by-Side] 错误: 模型 {request.model_a_id} 不存在")
            raise HTTPException(status_code=404, detail=f"模型 {request.model_a_id} 不存在")
        if not model_b_info:
            print(f"[Side-by-Side] 错误: 模型 {request.model_b_id} 不存在")
            raise HTTPException(status_code=404, detail=f"模型 {request.model_b_id} 不存在")
        
        print(f"[Side-by-Side] 模型验证通过")
        
        # 获取或创建会话
        print(f"[Side-by-Side] 处理会话...")
        ...
        try:
            ...
            print(f"[Side-by-Side] 模型调用完成")
        except asyncio.TimeoutError:
            print(f"[Side-by-Side] 模型调用超时（90秒）")
            raise HTTPException(status_code=504, detail="模型调用超时，请稍后重试")
        except Exception as e:
            print(f"[Side-by-Side] 模型调用失败: {str(e)}")
            traceback.print_exc()
            raise HTTPException(status_code=500, detail=f"模型调用失败: {str(e)}")
        
        # 更新会话历史
        print(f"[Side-by-Side] 更新会话历史...")
        messages.append({
            "role": "assistant",
        ...
        print(f"[Side-by-Side] 提交数据库...")
        await db.commit()
        print(f"[Side-by-Side] 准备返回响应...")
        ...
        # 捕获所有其他异常，确保返回响应
        print(f"[Side-by-Side] 未预期的错误: {str(e)}")
        traceback.print_exc()

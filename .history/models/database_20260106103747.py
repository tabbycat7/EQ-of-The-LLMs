...
async def init_db():
    """初始化数据库"""
    from .schemas import Battle, Vote, ModelRating, ChatSession, SideBySideVote, User, BattleEvaluation, BattleRecord
    ...
    async with engine.begin() as conn:
        ...
        # 为 battle_records 表添加新的评价维度字段（5个维度）
        database_url_str = str(engine.url)
        is_sqlite = "sqlite" in database_url_str
        ...
        for field_name, field_type in new_dimension_fields:
            try:
                await conn.execute(text(f"ALTER TABLE battle_records ADD COLUMN {field_name} {field_type}"))
                print(f"Added field: {field_name}")
            except Exception:
                pass  # 列已存在，忽略错误
    
    # 不再执行数据迁移，直接使用新表
    # await migrate_to_battle_records()
    print("Skip old data migration")
...
async def migrate_to_battle_records():
    ...
    async with async_session_maker() as session:
        ...
        try:
            ...
            if existing_records:
                print("battle_records 表已有数据，跳过迁移")
                return
            ...
            if not battles:
                print("没有需要迁移的 battle 数据")
                return
            
            print(f"开始迁移 {len(battles)} 条 battle 记录...")
            ...
            await session.commit()
            print(f"成功迁移 {len(battles)} 条记录到 battle_records 表")
